...
<?php }
    ...
    <div class="panel panel-default" style="padding: 5px; margin-top: -20px">
                ...
                <tr id="tr">
                    <th style=" margin-top: 17px; padding: 5px 5px 5px 0px  scope='row'">
                        <?php \yii\bootstrap\Modal::begin([
                            'header' => '<h2>'.$sot->tovar_nom.'</h2>Narhi <b style="color:red">'.$sot->sotish.'</b> so`m</h4>',
                            'id' => $sot->ids,'class' => 'okno_k','size' => 'modal-lg',
                            'toggleButton' => ['label' => $t,'id' => 'sizeqr','tag'=>'button','class' => 'k_t_nom'.$sot['id'],
                            ],
                            'footer' => '<button type="button" id="'.$sot['id'].'" style="font-size: 30px"  class="Saqlash_qr">Buyurtma</button>',
                        ]);
                        ?>
                        ...
                        <div class="quantity">
                            ...
                            <input id="sizep" style="width: 100px; margin:  0px 5px 0px 5px; text-align: center" type="number" name="asosiyson" value="<?=$sot['kol']?>" class="kol_new<?=$sot['id']?>">
        ...
        <table>
            <tr >
                ...
                <td style="padding: 1px;"><p style="padding: 1px;background-color: red;color: white;height:37px;margin-top: 20px;font-size: 20px"><b><?= $c?></b></p></td>
    ...
    <input  type="text"  value="" class="lastpapka" name="lastpapka">
    </div>
...
<div style="background-color: white; padding: 1px" id='divtovar'>
   ...
   ?>
    ...
    <?php $i=0; foreach ($query as $ite ){ $i++?>
        ...
        if(($papkanom <> $ite['papkanom'] and $ite['papkanom']!='Tovar') and $papka_one == 0 ){$papka_one = 1;}
        if(($papkanom == $ite['papkanom'] and $ite['papkanom']!='Tovar') and $papka_one == 1){$papka_one = 0;}
        ...
        if($papka_one == 1  | $papka_one == 2){?>
            ...
            {
            echo '<button type="button" id="'.$ite['papka'].'"  class="papka">'.$ite['papkanom'].'</button>';
            }
            ?>
            </td>
...
$('.Saqlash').on('click', function(e){
    ...
    $.ajax({url: '<?php echo Yii::$app->request->baseUrl. '/site/tovaradd' ?>' ,
        ...
        success: function(data){
            if(data==0){alert('Tovar soni kam');}
            else{
