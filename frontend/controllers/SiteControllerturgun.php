...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $date = date("Y-m-d");
        $s = Asos::find()->where(['sana'=>$date])->andWhere(['diler_id'=>0])->andWhere(['client_id'=> Yii::$app->getUser()->identity->client_id])->andWhere(['user_id'=> Yii::$app->getUser()->identity->id])->one();
        if($s['sana'] == null){
    ...
    public function actionKarzina(){
        ...
        if(Yii::$app->request->post('asosiyson') || Yii::$app->request->post('ichkison')) {
            ...
            if($asosiy==null){
                ...
                $model->kol_ost = $idsup = Yii::$app->request->post('idsup');
            }
            if($ichki==null){
                $model->kol_in_ost = 0;
            }
            else{
                $model->kol_in_ost =$idsup = Yii::$app->request->post('idsup');
            }
            if($asosiy==null){
                $model->kol = 0;
            }
            ...
            $sotil = AsosSlave::find()->where(['asos_id'=>$s['id']])->andWhere(['user_id'=> Yii::$app->getUser()->identity->id])->orderBy(['id'=>SORT_DESC])->all();
...
//        echo Yii::$app->request->post('date1').' ----- '.Yii::$app->request->post('date2').' '.Yii::$app->request->post('haridor');
        ...
        */
        ...
        $query = Asos::find()->where('del_flag=1')->andWhere('tur_oper=2')->andWhere(['h_id' => $haridor])->andWhere(['between', 'sana', $date1, $date2]) ->andWhere(['client_id'=> Yii::$app->getUser()->identity->client_id]);
    ...
    public function actionKun()
    {
        ...
        $s = Asos::find()->where(['sana'=>$date])->andWhere(['diler_id'=>1]) ->andWhere(['client_id'=> Yii::$app->getUser()->identity->client_id])->all();
